...
public class UserController {
    ...
    public ResponseEntity<?> login(@RequestBody UserDto userDto) {
        try {
            ...
            if (user == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("O usuário não está cadastrado no sistema");
            }
    ...
    @PutMapping("/user/{userId}")
        public ResponseEntity<?> editUser(@PathVariable UUID userId,@RequestBody @Valid UserDto userDto){
       UserModel user = userSevice.updateUser(userId,userDto);
    ...
    @DeleteMapping("/user/{userId}")
        ...
        userSevice.deleteUser(uuid);
        return ResponseEntity.status(HttpStatus.OK).body(userSevice.findUserById(uuid).getCompany().getEmployeelist());
    }
